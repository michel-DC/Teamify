...
export const usePolling = (options: UsePollingOptions = {}) => {
  ...
  const startPolling = useCallback(() => {
    if (isPollingRef.current) {
      console.log("⏸️ [usePolling] Polling already active, skipping start");
      return;
    }
   ...
   */
  useEffect(() => {
    console.log("🔄 [usePolling] Auto-connect effect:", {
      autoConnect,
      isAuthenticated,
    ...
    if (autoConnect && isAuthenticated && currentUser?.uid) {
      console.log(
        "🚀 [usePolling] Starting polling for user:",
        currentUser.uid
      );
      startPolling();
    } else {
      console.log("⏸️ [usePolling] Not starting polling:", {
        autoConnect,
        isAuthenticated,
